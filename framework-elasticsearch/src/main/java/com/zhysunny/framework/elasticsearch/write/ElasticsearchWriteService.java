package com.zhysunny.framework.elasticsearch.write;
 ...
 */
public abstract class ElasticsearchWriteService<E> {

    private static final Logger LOGGER = LoggerFactory.getLogger(ElasticsearchWriteService.class);
    ...
    public ElasticsearchWriteService(String index, String type) {
        this.index = index;
        this.type = type;
    }

    public ElasticsearchWriteService() {
    ...
    public final int request(BulkRequestBuilder bulkRequest) {
        ...
        if (num > 0) {
            ...
            if (num - error > 0) {
                LOGGER.info("{}入库成功记录数：{}", Thread.currentThread().getName(), num - error);
            }
            if (error > 0) {
                LOGGER.info("{}入库异常记录数：{}", Thread.currentThread().getName(), error);
            }
        }
        return num;
    }
